...
const Insights = () => (
  <section className={`${styles.paddings} relative z-10`}>
    <motion.div variants={staggerContainer} initial="hidden" whileInView="show" viewport={{ once: 'false', amount: 0.25 }} className={`${styles.innerWidth} mx-auto flex flex-col`}>
      ...
      <TypingText tittle="| Insight" textStyles="text-center" />
      <TitleText tittle="Insight About Metaversa" textStyles="text-center" />
      <div className="mt-[50px] flex flex-col gap-[30px]">
        {
          insights.map((insight, index) => (
            <InsightCard kety={`insight-${index}`} {...insight} index={index + 1} />
          ))
        }
      </div>
